...
const DoctorSignup = ({ isLoggedIn, setisLoggedIn, setuserid }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://35.154.145.51:5000/api/v1/doctor/signup",
        {
          method: "POST",
          headers: {
        ...
        <div className="w-[500px] bg-white rounded-lg drop-shadow-lg">
          <div className="space-y-2 py-8 px-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 md:space-y-4" action="#">
              ...
              <div>
                <label htmlFor="email" className="block mb-2 text-base font-medium text-gray-900">Experience</label>
                <input value={experience} onChange={(e) => setExperience(e.target.value)} type="number" name="number" id="number" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="name@example.com" required="" />
              </div>
              <div>
