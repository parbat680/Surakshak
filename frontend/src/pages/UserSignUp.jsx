...
const UserSignUp = ({ isLoggedIn, setisLoggedIn, setuserid }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (age < 55) {
      toast.error("Age Should be greater than or equal to 55")
        ...
        <div className="w-[500px] bg-white rounded-lg drop-shadow-lg">
          <div className="space-y-2 py-8 px-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 md:space-y-4" action="#">
              ...
              <div>
                <label htmlFor="age" className="block mb-2 text-base font-medium text-gray-900">Age (Must be greater than or equal to 55)</label>
                <input value={age} onChange={(e) => setAge(e.target.value)} type="number" name="age" id="age" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Must be greater than or equal to 55" required="" />
              </div>
              <div>
